...
SERVICE_VERSION=2.2.0.BUILD-SNAPSHOT
...
if [[ "${MODE}" != "standalone" && "${PROFILE}" != "standalone" ]]; then
    # enabled monitor
    JAVA_OPT="${JAVA_OPT} -javaagent:/acmedcare/applications/profile-env/perfino/agent/perfino.jar=server=jvm.perfino.acmedcare.com,pool=remoting-master/${PROFILE} "
